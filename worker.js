...
app.get('/csrf-token', async (c) => {
  try {
    ...
    const response = c.json({ csrfToken });
...
app.post('/auth/signup', async (c) => {
  try {
    ...
    response.headers.set('Set-Cookie', `accessToken=${token}; Path=/; Domain=.cognitiev.com; HttpOnly; Secure; SameSite=Lax`);
...
app.post('/auth/verify-otp', async (c) => {
  try {
    ...
    const domain = isLocalhost ? 'localhost' : '.cognitiev.com';

    const cookieOptions = domain === 'localhost'
      ? `accessToken=${token}; Path=/; HttpOnly; SameSite=Lax`
      : `accessToken=${token}; Path=/; Domain=${domain}; HttpOnly; Secure; SameSite=Lax`;
...
app.post('/auth/logout', async (c) => {
  ...
  response.headers.set('Set-Cookie', 'accessToken=; Path=/; Domain=.cognitiev.com; HttpOnly; Secure; SameSite=Lax; Max-Age=0');
  return response;
});
...
app.post('/api/assistants', authenticate, async (c) => {
  ...
  try {
    ...
    return c.json({ message: 'Assistant created', assistant: newAssistant }, 201);
  } catch (err) {
...
app.put('/api/assistants/:id', authenticate, async (c) => {
  ...
  try {
    ...
    return c.json({ message: 'Assistant updated', assistant: updatedAssistant });
  } catch (err) {
...
// VoiceAI route
app.get('/api/assistant/:id', authenticate, async (c) => {
  const { id } = c.req.param();
  const env = c.get('env');
  const VAPI_API_KEY = env.VAPI_API_KEY;

  if (!VAPI_API_KEY) {
    return c.json({ message: 'VoiceAI API key not set in environment variables' }, 500);
  }
